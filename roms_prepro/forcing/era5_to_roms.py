...
def _read_nc_files(file_list, var_names):
    ...
    data = {vn: {'lon': None, 'lat': None, 'time': [], 'data': []} for vn in var_names}
    for fpath in file_list:
        try:
        ...
        if time is not None and hasattr(time, 'dtype') and 'datetime' in str(time.dtype):
            ...
            try:
                ...
                t = np.arange(vdata.shape[0])
            else:
                if tv > 1e9:
                    # epoch秒
                    t = np.array([datetime(1970,1,1) + timedelta(seconds=float(x)) for x in time])
                elif tv > 100000:
                    # hours since 1900 (ERA5常用)
                    t = np.array([datetime(1900,1,1) + timedelta(hours=float(x)) for x in time])
                elif tv > 100:
                    # hours since 1900 (较小值)
                    t = np.array([datetime(1900,1,1) + timedelta(hours=float(x)) for x in time])
                else:
                    # days since base_date (ROMS输出格式，如0~30)
                    t = np.array([datetime(1990,1,1) + timedelta(days=float(x)) for x in time])
        else:
            t = None
...
class ERA5toROMS:
    ...
    def _write_nc(self, filename, variables_dict):
        ...
        # 所有变量共用一个 time 维度
        max_ntime = max(len(v['roms_time']) for v in variables_dict.values())
        ncid.createDimension('time', max_ntime)

        # 每个变量写入
        for var in variables_dict.values():
            roms_time = var['roms_time']
            ...
            if time_var_name not in ncid.variables:
                ...
                tid[:len(roms_time)] = roms_time

            # 数据变量（维度是 'time', 'Jm', 'Im'）
            data = var['data']  # 已经是 (ntime, Lp, Mp) 格式
    ...
    def process(self):
        ...
        # 2. 分离grib和nc文件
        grib_files = [f for f in files if os.path.splitext(f)[1].lower() in ('.grib', '.grb', '.grib2')]
